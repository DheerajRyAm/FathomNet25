...
async def process_dataset(dataset_path: Path, output_dir: Path, max_concurrent_downloads: int = 10) -> None:
    ...
    test_bool = 'test' in dataset_path.name

    semaphore = asyncio.Semaphore(max_concurrent_downloads)

    async def download_image_limited(client: AsyncClient, url: str, output_path: Path) -> None:
    ...
    async with AsyncClient() as client:
        download_tasks = [
            download_image_limited(client, image.coco_url, image_paths[image.id])
            for image in dataset.images
        ]
        for task in tqdm_async.as_completed(download_tasks, total=len(download_tasks), desc="Downloading images"):
            await task
    ...
    for annotation in tqdm_sync(dataset.annotations, desc="Processing annotations"):
        ...
        label = None if test_bool else category_names[annotation.category_id]
        rows.append([str(roi_path.resolve()), label])
...
def main() -> None:
    ...
    parser.add_argument("dataset_path", type=Path, help="Path to the COCO dataset YAML file.")
    parser.add_argument("output_dir", type=Path, help="Base directory for the output files.")
    parser.add_argument("-n", "--num-downloads", type=int, default=10, help="Maximum number of concurrent downloads.")
    parser.add_argument("-v", "--verbose", action="count", default=0, help="Increase verbosity. Use -vv for debug level.")
    ...
    try:
        asyncio.run(process_dataset(args.dataset_path, args.output_dir, args.num_downloads))
